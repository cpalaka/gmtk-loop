...
## inversely related to how slowly the direction vector is eased normally
@export var direction_easing_coefficient_normal: float = 50.0
## inversely related to how slowly the direction vector is eased normally
@export var direction_easing_coefficient_web: float = 20.0
...
var current_easing_coefficient: float = direction_easing_coefficient_normal
...
func _process(delta: float) -> void:
	...
	dir_change_time += delta * current_easing_coefficient
	...
	direction_vector = player_vec.direction_to(mouse_vec)
...
#TODO: find way to decouple all 'create_web' and other actions logic
func track_player_travel():
	if Input.is_action_pressed("create_web"):
		current_easing_coefficient = direction_easing_coefficient_web
	...
	if Input.is_action_just_released("create_web"):
		...
		current_web_id += 1
		current_easing_coefficient = direction_easing_coefficient_normal
